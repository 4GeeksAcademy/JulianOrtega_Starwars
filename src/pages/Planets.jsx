...
export const Planetas = () => {
  ...
  useEffect(() => {
    const savedLikes = localStorage.getItem('likedPlanets');
    if (savedLikes) {
  ...
  const fetchPlanets = async (page) => {
    try {
      setIsLoading(true);
      const response = await fetch(`${API_URL}planets?page=${page}&limit=20`, {
      ...
      dispatch({
        type: 'planets',  
        payload: page === 1 ? data.results : [...store.planets, ...data.results]
      });
  ...
  return (
    <div className="d-flex flex-wrap gap-4 justify-content-center p-4">
      {store.planets?.map((planet, index) => {
        const isLiked = likedUids.includes(planet.uid);
        const isLastItem = store.planets.length === index + 1;
        
        const cardContent = (
          <>
            <img
              src={planet.imageUrl || "https://media.sproutsocial.com/uploads/2022/06/profile-picture.jpeg"}
            ...
            >
              ...
              <div className="d-flex flex-row justify-content-between align-items-center w-100">
                <Link to={`/planets/${planet.uid}`} className="text-decoration-none text-danger small">
                  Databank
                </Link>
                <div 
                  className={`
